...
def get_table_content(tableXPATH):
    ...
    table_th = []  #存放table数据
    table_td = []  #存放table数据
    for th in table_tr_list:  #遍历每一个tr
        #将每一个tr的数据根据td查询出来，返回结果为list对象
        table_td_list = th.find_elements(By.TAG_NAME, "th")
        row_list = []
        print(table_td_list)
        for td in table_td_list:  #遍历每一个td
            row_list.append(td.text)  #取出表格的数据，并放入行列表里
        if len(row_list)!=0:
            table_th.append(row_list)
    for tr in table_tr_list:  #遍历每一个tr
        ...
        for td in table_td_list:  #遍历每一个td
            row_list.append(td.text)  #取出表格的数据，并放入行列表里
        if len(row_list)!=0:
            table_td.append(row_list)
    return list(zip(table_th,table_td))
...
"""
def get_table_contentCss(cssSelector):
  locator = cssSelector + ">tbody>tr"
  table_list = []
  table_tr_list = driver.find_elements(By.CSS_SELECTOR, locator)[0:] #去掉表头
...
driver.find_element_by_xpath('/html/body/table[3]/tbody/tr[3]/td/a').click()#QualityRateAnalysis 
...
QualityRate = get_table_content("/html/body/table")#获取OEEAnalysis表格内容
print(QualityRate)
#QualityRate = get_table_contentCss("#table_trend")#获取OEEAnalysis表格内容
